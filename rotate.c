...
void set_options(char ** args) {
	...
	while(arg != NULL) {
		if(0 == strncmp(arg,"debug",5)) {
			...
			if(debug) printf("USING BRIGHT CONTROL\n");
			change_brightness=1;
		}
		arg=(char*)*(args+i++);
	}
...
void packet_loop() {
	...
	while(1) {
		if(!screen_locked() && !screen_dimmed()) {
			...
			if(debug) {
				switch(current_pos) {
					case UPRIGHT: current_orientation="UPRIGHT"; break;
					case UPSIDEDOWN: current_orientation="UPSIDEDOWN"; break;
					case LEFT: current_orientation="LEFT"; break;
					case RIGHT: current_orientation="RIGHT"; break;
					default: current_orientation="INIT"; break;
				}
			}
			...
			if(debug) {
				switch(pos) {
					case UPRIGHT: orientation="UPRIGHT"; break;
					case UPSIDEDOWN: orientation="UPSIDEDOWN"; break;
...
int get_position(int x, int y, int z) {
	...
	if (total_accel > 1200000) {
		if(debug) printf(" too much shaking\n");
		return(-2);
	}
...
void rotate(int pos) {
	...
	switch(pos) {
		case UPRIGHT:		r_to=RR_Rotate_0	; do_rotate=1 ; break;
		case RIGHT:		r_to=RR_Rotate_90	; do_rotate=1 ; break;
		case UPSIDEDOWN:	r_to=RR_Rotate_180	; do_rotate=1 ; break;
		case LEFT:		r_to=RR_Rotate_270	; do_rotate=1 ; break;
		default: break;
	}
